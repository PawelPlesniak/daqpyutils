...
from daqpyutils.repository_handling.utils import (
    ...
    copy_template,
    format_application_name_to_file_name,
    ingest_item_list,
    ...
    parse_applications,
    populate_template,
    replace_file_path_with_dot_scope_notation,
    setup_dot_git,
...
def test_item_is_formatted_with_version() -> None:
    ...
    assert (
        item_is_formatted_with_version("daqpyutils=1.0.0-beta+build.123+build..456")
        is False
    )
...
def test_validate_item_format_against_type() -> None:
    """
    Test the validate_item_format_against_type function with various item types and
    formats.
    """  # noqa: D205
...
def test_strip_version_from_package_name() -> None:
    """
    Test the strip_version_from_package_name function with valid and invalid package
    names.
    """  # noqa: D205
    assert strip_version_from_package_name("daqpyutils==1.0.0") == "daqpyutils"
    # Invalid formats
    strip_version_from_package_name("daqpyutils-1.0.0") == "daqpyutils-1.0.0"  # noqa: B015
    strip_version_from_package_name(  # noqa: B015
        "daqpyutils=1.0.0-beta+build..123"
    ) == "daqpyutils=1.0.0-beta+build..123"
...
def test_unpack_items() -> None:
    ...
    with tempfile.NamedTemporaryFile(delete=True) as temp_file:
        ...
        unpacked_items_from_file = unpack_items(
            "requirements", items_file=temp_file_path
        )
        ...
        all_unpacked_items = unpack_items(
            "requirements", items=requirements_list, items_file=temp_file_path
        )
    ...
    with pytest.raises(SystemExit):
        unpack_items(
            "applications", items=["application_names_should_be_in_kebab_case"]
        )


def test_validate_compliance_with_naming_conventions() -> None:
    """
    Test the validate_compliance_with_naming_conventions function with valid and invalid
    package names.
    """  # noqa: D205
    # Valid package names
    ...
    validate_compliance_with_naming_conventions(
        "daqpyutils", ["create-python-dunedaq-application", "some-test-application"]
    )
    ...
    with pytest.raises(SystemExit):
        validate_compliance_with_naming_conventions(
            "daqpyutils_1.0.0-beta+build..123..456", []
        )

    # Invalid application names
    with pytest.raises(SystemExit):
        validate_compliance_with_naming_conventions(
            "daqpyutils", ["create_python_dunedaq_application"]
        )

    # Invalid combination
    with pytest.raises(SystemExit):
        validate_compliance_with_naming_conventions(
            "daqpyutils_1..0", ["create_python_dunedaq_application"]
        )


def test_setup_dot_git(tmp_path: Path) -> None:
    """
    Test the setup_dot_git function by creating a temporary directory and checking if
...
def test_make_subdirs(tmp_path: Path) -> None:
    """
    Test the make_subdirs function by creating a temporary directory and checking if the
    specified subdirectories are created.
    """  # noqa: D205
    # Create a temporary directory
    temp_dir = tmp_path / "test_repo"
    temp_dir.mkdir()
...
def test_populate_template(tmp_path: Path) -> None:
    """
    Test the populate_template function by creating a temporary directory and checking
    if the template files are populated correctly.
    """  # noqa: D205
    # Create a temporary directory
    temp_dir = tmp_path / "test_repo"
    temp_dir.mkdir()
    ...
    populate_template(
        template_file, {"test_var": "THIS_SHOULD_NOT_BE_PRESENT"}, output_path
    )
...
def test_copy_template(tmp_path: Path) -> None:
    """
    Test the copy_template function by creating a temporary directory and checking if
    the template files are copied correctly.
    """  # noqa: D205
    # Create a temporary directory
    temp_dir = tmp_path / "test_repo"
    temp_dir.mkdir()
    ...
    assert (
        output_path.exists() and gitignore_template_content in output_path.read_text()
    )


def test_construct_default_readme_md(tmp_path: Path) -> None:
    """
    Test the construct_default_readme_md function by creating a temporary directory and
    checking if the README.md file is created correctly.
    """  # noqa: D205
    # Create a temporary directory
...
def test_construct_default_gitignore(tmp_path: Path) -> None:
    """
    Test the construct_default_gitignore function by creating a temporary directory and
    checking if the .gitignore file is created correctly.
    """  # noqa: D205
    # Create a temporary directory
    temp_dir = tmp_path / "test_repo"
    temp_dir.mkdir()
...
def test_construct_default_dot_github(tmp_path: Path) -> None:
    """
    Test the construct_default_dot_github function by creating a temporary directory and
    checking if the .github directory is created correctly.
    """  # noqa: D205
    # Create a temporary directory
    temp_dir = tmp_path / "test_repo"
    temp_dir.mkdir()
...
def test_construct_default_pre_commit_config_yaml(tmp_path: Path) -> None:
    """
    Test the construct_default_pre_commit_config_yaml function by creating a temporary
    directory and checking if the .pre-commit-config.yaml file is created correctly.
    """  # noqa: D205
    # Create a temporary directory
    temp_dir = tmp_path / "test_repo"
    temp_dir.mkdir()
...
def test_construct_application_file(tmp_path: Path) -> None:
    """
    Test the construct_application_file function by creating a temporary directory and
    checking if the application file is created correctly.
    """  # noqa: D205
    # Create a temporary directory
    repo_name = "test_repo"
    ...
    construct_application_file(
        application_name,
        temp_dir / "src" / repo_name / "apps" / f"{application_name}.py",
    )
...
def test_format_application_name_to_file_name() -> None:
    """
    Test the format_application_name_to_file_name function with valid application names
    only, as earlier exceptions are designed to catch improperly formatted names.
    """  # noqa: D205
    assert (
        format_application_name_to_file_name("test-application")
        == "test_application.py"
    )
    assert (
        format_application_name_to_file_name("another-test-application")
        == "another_test_application.py"
    )


def test_replace_file_path_with_dot_scope_notation() -> None:
    """Test the replace_file_path_with_dot_scope_notation function with valid file paths."""  
    assert (
        replace_file_path_with_dot_scope_notation("test_repo/apps/test_application.py")
        == "test_repo.apps.test_application"
    )
    assert (
        replace_file_path_with_dot_scope_notation(
            "test_repo/apps/another_test_application.py"
        )
        == "test_repo.apps.another_test_application"
    )
...
def test_parse_applications(tmp_path: Path) -> None:
    ...
    # DEBUGGING: Print list of files in app dir
    ...
    for app_name in application_names:
        app_file = (
            ...
            / repo_name
            / "apps"
            / format_application_name_to_file_name(app_name)
        )
    ...
    for app_name in application_names:
        ...
        assert (
            f'{app_name} = "{repo_name}.apps.{app_name.replace("-", "_")}:main"'
            in application_entry_points
        )
...
def test_construct_default_pyproject_toml(tmp_path: Path) -> None:
    """
    Test the construct_default_pyproject_toml function.
    
    Create a temporary directory and check the pyproject.toml file.
    """
    # Create a temporary directory
    repo_name = "test_repo"
    temp_dir = tmp_path / repo_name
    ...
    construct_default_pyproject_toml(
        temp_dir, package_description, requirements, applications, strict
    )
    ...
    assert (
        ...
        and "}}" not in pyproject_file.read_text()
    ...
    for application in applications:
        assert (
            f'{application} = "{repo_name}.apps.{application.replace("-", "_")}:main"'
            in pyproject_file.read_text()
        )


def test_make_files(tmp_path: Path) -> None:
    """
    Test the make_files function.

    Create a temporary directory and check if the files are created correctly.
    """
    # Create a temporary directory
    repo_name = "test_repo"
    temp_dir = tmp_path / repo_name
